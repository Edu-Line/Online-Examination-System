...
function update_check(sq_version){
	...
	$.ajax({
		 type: "POST",
		 data : formData,
			url: "http://update.eduline.co/",
		success: function(data){
			if(data.trim()==sq_version){
			var msg="<div class='alert alert-success'>You are using updated version of <a href='http://eduline.co'>Edu-Line "+sq_version+"</a></div>";	
			}else{
			var msg="<div class='alert alert-danger'>New version available: Edu-Line v"+data.trim()+". You are using outdated version of Edu-Line v"+sq_version+". Visit <a href='http://eduline.co'>www.eduline.co</a> to download</div>";	
